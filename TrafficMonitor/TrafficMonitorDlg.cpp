...
bool CTrafficMonitorDlg::IsTemperatureNeeded() const
{
    ...
    return theApp.m_general_data.IsHardwareEnable(HI_CPU) || theApp.m_general_data.IsHardwareEnable(HI_GPU) || theApp.m_general_data.IsHardwareEnable(HI_HDD) || theApp.m_general_data.IsHardwareEnable(HI_MBD);
}
...
UINT CTrafficMonitorDlg::MonitorThreadCallback(LPVOID dwUser)
{
    ...
    if ((pThis->m_in_bytes == 0 && pThis->m_out_bytes == 0) || (pThis->m_last_in_bytes == 0 && pThis->m_last_out_bytes == 0) || pThis->m_connection_change_flag || pThis->m_last_in_bytes > pThis->m_in_bytes || pThis->m_last_out_bytes > pThis->m_out_bytes)
    {
        cur_in_speed = 0;
        cur_out_speed = 0;
    }
...
void CTrafficMonitorDlg::OnTimer(UINT_PTR nIDEvent)
{
    ...
    if (nIDEvent == MAIN_TIMER)
    {
        ...
        // 根据任务栏颜色自动设置任务栏窗口背景色
        if (theApp.m_taskbar_data.auto_set_background_color && theApp.m_win_version.IsWindows8OrLater() && IsTaskbarWndValid() && theApp.m_taskbar_data.transparent_color != 0 && !m_is_foreground_fullscreen)
        {
            CRect rect;
            ...
            if (pointx < 0)
                pointx = 0;
            if (pointx >= m_screen_size.cx)
                pointx = m_screen_size.cx - 1;
            if (pointy < 0)
                pointy = 0;
            if (pointy >= m_screen_size.cy)
                pointy = m_screen_size.cy - 1;
...
void CTrafficMonitorDlg::OnInitMenu(CMenu *pMenu)
{
    ...
    {
    case 100:
        pMenu->CheckMenuRadioItem(ID_TRANSPARENCY_100, ID_TRANSPARENCY_40, ID_TRANSPARENCY_100, MF_BYCOMMAND | MF_CHECKED);
        break;
    case 80:
        pMenu->CheckMenuRadioItem(ID_TRANSPARENCY_100, ID_TRANSPARENCY_40, ID_TRANSPARENCY_80, MF_BYCOMMAND | MF_CHECKED);
        break;
    case 60:
        pMenu->CheckMenuRadioItem(ID_TRANSPARENCY_100, ID_TRANSPARENCY_40, ID_TRANSPARENCY_60, MF_BYCOMMAND | MF_CHECKED);
        break;
    case 40:
        pMenu->CheckMenuRadioItem(ID_TRANSPARENCY_100, ID_TRANSPARENCY_40, ID_TRANSPARENCY_40, MF_BYCOMMAND | MF_CHECKED);
        break;
    default:
        break;
    }
